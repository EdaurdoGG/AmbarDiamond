...
SELECT 
    ...
    (SELECT IFNULL(SUM(dv2.Total),0) 
     FROM DetalleVenta dv2 
     WHERE dv2.idVenta = v.idVenta) AS SubtotalVenta,
    (SELECT IFNULL(SUM(dv2.Total*0.16),0) 
     FROM DetalleVenta dv2 
     WHERE dv2.idVenta = v.idVenta) AS IVAVenta,
    (SELECT IFNULL(SUM(dv2.Total + dv2.Total*0.16),0) 
     FROM DetalleVenta dv2 
...
JOIN Persona emp ON v.idPersona = emp.idPersona           -- ✅ empleado (quien realiza la venta)
LEFT JOIN Persona cli ON cli.idRol = 3                    -- ❌ cliente opcional, si tienes rol 3 como cliente
;
...
SELECT 
    ...
    dv.IVA,
    dv.Total AS Subtotal,
    -- Totales calculados en tiempo real desde DetalleVenta
    (SELECT IFNULL(SUM(dv2.Total),0) FROM DetalleVenta dv2 WHERE dv2.idVenta = v.idVenta) AS SubtotalVenta,
    (SELECT IFNULL(SUM(dv2.Total*0.16),0) FROM DetalleVenta dv2 WHERE dv2.idVenta = v.idVenta) AS IVAVenta,
    (SELECT IFNULL(SUM(dv2.Total + dv2.Total*0.16),0) FROM DetalleVenta dv2 WHERE dv2.idVenta = v.idVenta) AS TotalVenta,
    v.TipoPago,
    v.Estatus
FROM Venta v
JOIN DetalleVenta dv ON v.idVenta = dv.idVenta
